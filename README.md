...
| **데이마켓 실시간** | US 연장거래(오버나잇) 시세를 **야후 WebSocket 스트리머**로 실시간 수신        |
...
| 독립 폴링        | 국내(15초) · 해외(1분) 개별 새로고침 간격 + 캐싱 (데이마켓은 WebSocket 실시간) |
...
| 실시간   | **WebSocket** (`ws`, Electron 메인)          | 야후 오버나잇 시세 — REST 폴링에서 피벗    |
...
└── shared/                  # 공유 인프라
    ...
    │   └── types.ts                 # 역추론 응답 타입
    ├── yahoo.ts                 # 야후 오버나잇(데이마켓) WS 응답 파싱/병합
...
대부분의 시세는 **REST 폴링**(국내 15초 · 해외 1분, 배치 + backoff)으로 가져오지만, **US 데이마켓(연장거래)** 구간만은 네이버가 시세를 제공하지 않습니다. 이 구간은 **야후 WebSocket 스트리머**로 실시간 수신합니다.

```
[REST 폴링]  네이버 증권 ──┐
                          ├─▶ 가격 병합 (야후는 데이마켓 가격/등락/상태만 차용)
[WebSocket]  야후 스트리머 ┘     → 세션 분류 → ViewModel → UI
             (Electron 메인 프로세스, ws)
```

- **세션 모델** — `MarketSession = REGULAR | PRE | AFTER | OVERNIGHT | CLOSED`. `OVERNIGHT`(데이마켓)만 야후 WS, 나머지는 네이버. NXT(넥스트레이드)가 정규장 폐장 후 우선 처리됩니다.
- **WebSocket을 메인 프로세스에 둔 이유** — Electron 메인(Node 20)에는 전역 `WebSocket`이 없어 `ws` 패키지를 도입했고, 단일 연결을 메인에서 관리해 렌더러 다중 구독 churn을 막습니다.
- **병합 원칙** — 가격·등락·세션 상태만 야후에서 차용하고 나머지 필드는 네이버 값을 유지합니다. 야후 미수신/stale 종목은 네이버 값으로 폴백합니다.
...
- 데이마켓 타겟 선정을 `selectDaymarketTargets` **단일 기준으로 통일** — 모든 경로가 같은 종목 집합을 구독하도록 해 churn 제거. (`hasExtendedHours`로 좁히지 않음)
...
| 실시간    | 야후 오버나잇(데이마켓) WS 스트리머 · 세션 분류 · 폴링+WS 가격 병합                           |
