...
| 독립 폴링         | 국내(15초) · 해외(1분) 개별 새로고침 간격 + 캐싱                  |
...
> feature 기반 구조를 선택한 이유는 [#1 설계 원칙 없이 AI에게 시키면 리팩토링으로 돌아옵니다](#1-설계-원칙-없이-ai에게-시키면-리팩토링으로-돌아옵니다)에서 다룹니다.
...
- "도메인 기반으로 구조를 바꿔라"고 지시했을 때, AI는 파일을 폴더별로 _이동만_ 했습니다
...
| 설계 원칙 · 아키텍처 | 프로젝트 초기 17개 규칙 → 실전 중 15개 추가 학습하여 **총 32개 원칙** 체계화, feature 기반 폴더 구조 · ViewModel 패턴 도입 시점 · 상태 분리 전략(Zustand + React Query) 판단                              |
| 버그 발견 · 디버깅   | 직접 사용하며 발견(화살표 반전, 드롭 안 됨, 환율 깨짐, 해상도 입력 스냅백, 시트 ESC 중첩 닫힘, 그리드/타일 레이아웃 깨짐) + "프리셋 클릭 안 되는데 currency만 작동"으로 transform hit-test 침범 원인 특정 |
...
| 품질 검증 워크플로우 | "시니어 리뷰어 페르소나" 워크플로우 도입 → 치명 이슈 7건 사전 발견 유도                                                                                                                                   |
| 배포 운영            | 사전 릴리즈를 통해 자동 업데이트 흐름 검증, gist 기반 공지사항 작성                                                                                                                                       |
...
| 문서                                                                    | 설명                                                           |
| ----------------------------------------------------------------------- | -------------------------------------------------------------- |
| [DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md)                               | 디자인 토큰 6단계 체계 · 색상/간격 규칙 전문                   |
| [ENFORCEMENT.md](docs/DESIGN_SYSTEM_ENFORCEMENT.md)                     | ESLint 커스텀 규칙 4개 · 강제 적용 방법                        |
| [VANILLA_EXTRACT_DESIGN_GUIDE.md](docs/VANILLA_EXTRACT_DESIGN_GUIDE.md) | 차기 프로젝트용 Vanilla Extract 기반 디자인 시스템 설계 기준서 |
