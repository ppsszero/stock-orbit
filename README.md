...
"AI에게 상세한 요구사항을 주면 프로덕션 수준의 React 프로젝트가 나오는가?"를 검증하기 위한 실험이었고, 결론부터 말하면 ***나오지 않았습니다.***
하지만 **사람이 설계를 잡고 AI가 구현을 담당하는 구조**에서는 가능했습니다.

### 이 프로젝트에서 확인한 것

> AI가 코드를 대신 써주는 시대가 됐지만, 결과물의 품질은 AI가 아니라 **요청을 설계하는 사람의 수준**에 의해 결정됩니다.
> 이 프로젝트를 마치며 남은 질문과 생각은 [회고](#회고--ai-시대에서-개발자의-역할)에 정리했습니다.
...
- [문제 해결](#문제-해결)
- [역할 분담](#역할-분담)
- [회고 — AI 시대에서 개발자의 역할](#회고--ai-시대에서-개발자의-역할)
...
| **미국 주식 실시간** | 미국 주식 시세를 **야후 WebSocket 스트리머**로 실시간 차용 (정규장·데이마켓 등 전 세션) |
| **시장 세션 분류** | 정규 · 프리장 · 애프터 · 데이마켓 · 장마감 자동 분기 (NXT 우선 처리)          |
| **3가지 뷰**    | 리스트 · 그리드 · 타일 히트맵 (크기=시가총액, 색=등락률)                    |
| **종목 검색**    | 한글/영문/코드 자동완성, 키보드 네비게이션                               |
| **글로벌 랭킹**   | 국가별 거래량/거래대금 상위 종목                                     |
| **투자 동향**    | 매매동향(KOSPI/KOSDAQ), 경제 캘린더, 금리(기준금리/국내금리)              |
| **뉴스**       | AI 브리핑, 메인 뉴스, 머니스토리                                   |
| **시장 지표**    | 지수 10개 + 금속 7개 + 에너지 5개 + 환율 4개 (마퀴 티커)                |
| **자동 업데이트**  | 인앱 모달 UI (진행률 바 + "지금 재시작") · 받기 전 확인 · 릴리즈 노트 · 건너뛰기  |
| **API 보호**   | 30개 종목 한도 + 10개 배치 폴링 + 429 backoff + jitter           |
| **그룹 정렬**    | 종목명 · 등락률 · 내 순서 (그룹 내 정렬, 설정 유지)                      |
| 드래그 정렬       | dnd-kit 기반, 그룹 내 이동만 허용                                |
| 프리셋          | 관심종목 그룹 관리 (추가/삭제/이름변경)                                |
| 독립 폴링        | 국내(15초) · 해외(1분) 개별 새로고침 간격 + 캐싱 (미국 주식은 WebSocket 실시간) |
| 종목 상세        | 앱 내 모달 + 웹뷰 — 데이마켓 US 종목은 네이버/야후 페이지 선택(기억하기)          |
| 웹뷰 광고 차단     | 종목 페이지 광고·트래커 도메인 차단 → 로드 속도 개선                        |
| 증시현황(섹터)     | 표시 개수 설정 (국내/해외 분리, 상한 안내 토스트)                         |
...
| 상태 관리 | **Zustand** (클라이언트) + **React Query** (서버) | 국내/해외 독립 폴링 + UI 상태 분리       |
| 스타일   | Emotion + **CSS Custom Properties**        | 테마 전환 시 리렌더 0건 달성            |
| DnD   | @dnd-kit                                   | HTML5 DnD가 그리드에서 불안정 + 성능 이점 |
| 테스트   | Vitest + RTL                               | 23파일 · 192 테스트               |
| 실시간   | **WebSocket** (`ws`, Electron 메인)          | 미국 주식 실시간 시세(전 세션) — REST 폴링에서 피벗 |
| API   | 네이버 증권 + 야후 (프록시 CORS 우회)                  | **공식 명세 없음** — 역추론 타입으로 대응   |
...
```
electron/
├── main.js                  # 메인 프로세스 — 창/트레이/자동 업데이트 + 야후 WebSocket 스트리머 + 광고 차단
└── preload.js               # IPC 화이트리스트 (렌더러 노출 API)

src/
├── app/                     # 앱 조합 레이어 (orchestration만)
│   ├── App.tsx  main.tsx        # 진입점
│   ├── SheetManager.tsx         # 시트/모달 라우팅
│   ├── StockViewSwitch.tsx      # 리스트/그리드/타일 뷰 전환
│   ├── layout/                  # TitleBar · StatusBar
│   ├── providers/               # QueryProvider (React Query)
│   └── store/                   # Zustand store + selectors
│
├── features/                # 도메인별 기능 (각 feature = components/ + hooks/ + utils/)
│   ├── stock/                   # 종목 — 3뷰, 폴링, ViewModel, DnD, 데이마켓 소스 선택
│   ├── search/                  # 종목 검색 자동완성
│   ├── news/  ranking/          # 뉴스(AI 브리핑) · 글로벌 랭킹
│   ├── investor/  marquee/      # 투자정보(매매동향·캘린더·금리) · 시장지표 마퀴 티커
│   ├── preset/  settings/       # 관심종목 그룹 · 설정
│   └── screenshot/  notice/     # 스크린샷 캡처 · 공지사항(Gist)
│
└── shared/                  # 공유 인프라
    ├── naver/                   # 네이버 증권 API — 도메인별 분리
    │   ├── client.ts                # 공통 fetch (프록시 CORS 우회)
    │   ├── stocks.ts  indices.ts    # 종목 · 지수
    │   ├── polling.ts               # 배치 폴링 + 세션 분류 + backoff
    │   ├── news.ts  ranking.ts      # 뉴스 · 랭킹
    │   ├── investor.ts  calendar.ts # 매매동향 · 경제 캘린더
    │   ├── interest.ts  sectors.ts  # 금리 · 섹터(증시현황)
    │   ├── commodities.ts  fx.ts    # 원자재 · 환율
    │   ├── research.ts  urls.ts     # 투자 리서치 · URL 생성기
    │   └── types.ts                 # 역추론 응답 타입
    ├── yahoo.ts                 # 야후 미국 주식 실시간 WS 응답 파싱/병합(전 세션)
    ├── styles/                  # 디자인 토큰 6단계 (tokens→theme→vars→semantic→shared)
    ├── types/                   # 도메인 모델 (StockPrice, MarketSession 등)
    ├── ui/                      # 공통 컴포넌트 (Badge, Menu, Modal, SheetLayout …)
    ├── hooks/  utils/           # 공통 훅 · 순수 유틸 (포맷·DnD modifier 등)
```

> feature 기반 구조를 선택한 이유는 [#1 설계 원칙 없이 AI에게 시키면 리팩토링으로 돌아옵니다](#problem-1)에서 다룹니다.

### 실시간 데이터 파이프라인 — 폴링 + WebSocket 하이브리드

대부분의 시세는 **REST 폴링**(국내 15초 · 해외 1분, 배치 + backoff)으로 가져옵니다. 다만 **미국 주식**은 네이버 시세가 지연되고 데이마켓(연장거래) 구간은 아예 제공되지 않아, **야후 WebSocket 스트리머**로 가장 빠른 실시간가를 차용합니다 — 정규장·프리·애프터·오버나잇 **전 세션**. 갱신은 국내 사이클(15초)에 동기시켜 화면 전체가 한 박자에 깜빡이도록 했습니다.

```
[REST 폴링]  네이버 증권 ──┐
                          ├─▶ 가격 병합 (미국 주식은 야후가 가격/등락/세션 차용, 그 외 네이버)
[WebSocket]  야후 스트리머 ┘     → 세션 분류 → ViewModel → UI
             (Electron 메인 프로세스, ws)
```

- **세션 모델** — `MarketSession = REGULAR | PRE | AFTER | OVERNIGHT | CLOSED`. **미국 주식은 야후가 가격·세션 권한**(가장 정확·신속), 그 외(국내·비미국 종목·지수·환율)는 네이버. 가격과 세션을 한 출처에서 함께 가져와 "가격은 움직이는데 장마감 표시" 같은 불일치를 차단합니다. NXT(넥스트레이드)는 정규장 폐장 후 우선 처리됩니다.
- **WebSocket을 메인 프로세스에 둔 이유** — Electron 메인(Node 20)에는 전역 `WebSocket`이 없어 `ws` 패키지를 도입했고, 단일 연결을 메인에서 관리해 렌더러 다중 구독 churn을 막습니다.
- **병합 원칙** — 미국 주식은 가격·등락·세션을 야후에서 차용하고 나머지 필드(거래량·시총 등)는 네이버 값을 유지합니다. 야후 미수신/장마감 종목은 네이버 값으로 폴백합니다.

> 이 파이프라인을 만들며 부딪힌 함정(WS 끊김 오인 · 무스냅샷)은 [#7 WebSocket은 "연결됨"이 "데이터 도착"을 뜻하지 않습니다](#problem-7)에서 다룹니다.
...
| 7   | WebSocket "연결됨"을 "데이터 도착"으로 오인 → 끊김 오판·빈 화면 | 생명주기/세션 분기로 '연결중' 상태 분리, 단일 타겟 기준 통일  |


> 각 항목의 상세(Problem → Analyze → Action → Result)는 펼쳐서 볼 수 있습니다.
...
<details>
<summary><strong>#1 설계 원칙 없이 AI에게 시키면 리팩토링으로 돌아옵니다</strong></summary>
...
<details>
<summary><strong>처음에 전달한 설계 원칙 (17개)</strong></summary>
...
<details>
<summary><strong>프로젝트 중에 추가 학습한 원칙 (15개)</strong> — 이것들이 없어서 실제 버그가 발생했습니다</summary>
...
<details>
<summary><strong>#2 AI가 "완료"라고 해도 끝이 아닙니다 — 별도 리뷰 단계가 필수입니다</strong></summary>
...
**구현 모드 vs 리뷰 모드 — 왜 같은 AI가 다른 결과를 내는가**
...
<details>
<summary><strong>#3 CSS transform은 보이는 위치와 클릭 위치가 다릅니다</strong></summary>
...
<details>
<summary><strong>#4 비공식 API 폴링은 차단과의 전쟁입니다</strong></summary>
...
| 3   | 폴링 jitter ±20%              | 여러 사용자 동시 요청 스파이크 방지                            |
...
<details>
<summary><strong>#5 Emotion 테마 전환 시 전체 스타일이 재생성됩니다</strong></summary>
...
> 디자인 시스템 상세: [DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md) ·
> ESLint 강제 규칙: [ENFORCEMENT.md](docs/DESIGN_SYSTEM_ENFORCEMENT.md)
...
<details>
<summary><strong>#6 AI는 데이터를 "보고도" 단순화합니다 — 필드 매핑 확인이 필요합니다</strong></summary>
...
<details>
<summary><strong>#7 WebSocket은 "연결됨"이 "데이터 도착"을 뜻하지 않습니다</strong></summary>

> US 데이마켓(연장거래) 실시간 시세를 야후 WebSocket 스트리머로 가져오면서, REST 폴링에는 없던 두 가지 함정에 부딪혔습니다.

**Problem**

- 야후 WS는 **초기 스냅샷을 주지 않습니다.** 구독 직후엔 아무 메시지도 없고, 첫 틱이 도착해야 가격이 생깁니다. 그 사이 사용자는 **빈 값/장마감 종가**를 보게 됩니다.
- 연결이 잠깐 끊겼을 때 이를 **"데이터 끊김"으로 오인**해, 메인 프로세스 재시작이 필요한 상황과 일시적 재연결을 구분하지 못했습니다.
- 데이마켓 대상 종목을 `hasExtendedHours` 플래그로 좁혔더니, 플래그가 도착했다 사라졌다 하며 **구독/해지 churn(깜빡임)** 이 발생했습니다.

**Analyze**

- "연결(open)"과 "데이터 도착(first tick)"은 **다른 이벤트**입니다. open만으로 시세가 있다고 가정하면 안 됩니다.
- WS 끊김은 **두 종류** — (a) 일시적이라 자동 재연결로 복구되는 것, (b) 메인 프로세스 상태가 깨져 재시작이 필요한 것. 로그 메타로 구분해야 사용자에게 올바른 안내를 할 수 있습니다.
- 폴링 경로·15초 fast 경로·부트스트랩 경로가 **데이마켓 타겟 기준을 각자 다르게** 잡으면, 같은 종목이 경로마다 구독/해지를 반복합니다.

**Action**

- 첫 틱 도착 전까지는 **'연결중' 상태**를 별도로 표현 — `StatusDot`에 `pulse`(깜빡임)를 줘서 "끊김"이 아니라 "연결 중"임을 시각화 (`daymarketConnecting`).
- WS down 로그에 메타를 실어 **'끊김'과 '메인 재시작 필요'를 구분**.
- 타겟 선정을 `selectYahooTargets` **단일 기준으로 통일** — 모든 경로가 같은 종목 집합을 구독하도록 해 churn 제거. (`hasExtendedHours`로 좁히지 않음)
- 센티널(QQQ) 종목을 항상 동승시켜, 오버나잇 응답이 1건이라도 오면 **데이마켓 세션이 라이브임을 확정**.

**Result**

- 데이마켓 진입 시 빈 값/장마감 종가 대신 **'연결중' → 실시간가**로 자연스럽게 전환.
- 일시적 끊김에 "앱이 망가졌다"는 오해 제거.
- 경로 간 구독 churn(깜빡임) 사라짐.

> REST와 WebSocket은 **실패 양상이 다릅니다.** 폴링은 "응답이 늦거나 막히는" 문제지만, 스트리밍은 "연결은 됐는데 데이터가 아직 없는" 상태가 정상 흐름에 존재합니다. 이 차이를 UI 상태로 명시하지 않으면 사용자에게 거짓말을 하게 됩니다.
...
| 영역        | 한 일                                                                                        |
| --------- | ------------------------------------------------------------------------------------------ |
| 요구사항·설계   | 와이어프레임/API/인터랙션 명세 · **32개 설계 원칙** 체계화 · feature 구조·ViewModel·상태 분리 판단                     |
| 디버깅·검수    | 실사용 버그 발견 · "프리셋만 클릭 안 됨"으로 transform hit-test 원인 특정 · baseline/Optical Centering 등 시각 디테일 |
| API 방어 설계 | NXT 존재로 "폐장 skip" 기각 · 30개 한도 + 배치 폴링 전략 직접 설계                                             |
| 품질·배포     | "시니어 리뷰어 페르소나" 도입 → 치명 이슈 7건 사전 발견 · 사전 릴리즈 검증 · Gist 공지 · README 구조 설계                    |
...
| 영역     | 한 일                                                                   |
| ------ | --------------------------------------------------------------------- |
| 구현·API | 160개+ 소스(Electron~React) · 네이버 12개 API + 야후 WS 스트리머 · 역추론 타입          |
| 실시간    | 미국 주식 야후 WS 실시간(정규장+데이마켓 전 세션) · 세션 분류 · 폴링+WS 가격 병합              |
| 안정성·성능 | ErrorBoundary 3단·오프라인·IPC 가드·backoff·WS 재연결 · 동적 css()→정적 맵·CSS 변수 테마 |
| 테스트·배포 | Vitest+RTL 23파일·192 테스트 · electron-updater 자동 업데이트(받기 전 확인·건너뛰기)      |


> ***AI를 "코드 생성기"가 아니라 "설계 파트너"로 활용해야 합니다. 좋은 결과는 좋은 코드가 아니라, 좋은 설계 요청에서 나옵니다.***

---

## 회고 — AI 시대에서 개발자의 역할

이 프로젝트는 "AI에게 상세한 요구사항을 주면 프로덕션 수준의 결과물이 나오는가?"라는 질문에서 시작했습니다.

결론부터 말하면, **그냥 요구사항만 줄 때는 나오지 않았습니다.**
하지만 사람이 설계를 잡고 AI가 구현을 담당하는 구조에서는 가능했습니다.

AI는 코드 작성, 리뷰, 리팩토링, 테스트, 배포까지 전 과정을 수행할 수 있습니다.
하지만 결과물의 품질은 AI가 아니라 **요청을 설계하는 사람의 수준에 의해 결정됩니다.**

### 배경지식이 없는 요청은 '동작하는 코드'까지만 만듭니다

초기에는 원하는 기능 중심으로 요구사항을 전달했습니다.
...
- API 필드 검증 누락 → 특정 값(프리장 데이터) 누락
- 시가총액 단위 불일치 → 실사용 중 발견
- 설계 원칙 부재 → 3차례 리팩토링

이 문제들은 "코드를 못 짜서"가 아니라,
**요구사항을 구조적으로 정의하지 못해서** 발생했습니다.

### AI는 구현을 담당하고, 사람은 검증을 담당합니다

프로젝트 후반부에는 작업 방식이 바뀌었습니다.
...
같은 코드를 보고도 이걸 할 수 있는지 여부에 따라 결과물의 수준이 완전히 달라집니다.

### 중요한 건 '코딩 능력'이 아니라 '문제 정의 능력'입니다

이 프로젝트에서 가장 가치 있었던 건 코드가 아닙니다.

> "바탕화면에 주식 위젯을 띄우고 싶다"

이 단순한 필요를 실제 제품으로 만드는 과정에서 다음과 같은 질문들이 반복됐습니다:

- 어떤 데이터를 기준으로 보여줄 것인가
- 실시간성과 정확성 중 무엇을 우선할 것인가
- 상태를 어디서 관리해야 하는가
- 예외 상황을 어디까지 처리할 것인가

이 과정은 단순 구현이 아니라 **문제를 정의하고 쪼개는 작업**이었습니다.

### AI 시대에 줄어드는 것과 남는 것

AI는 이미 다음을 대체하고 있습니다:
...
반대로 여전히 사람에게 남아 있는 영역은 다음입니다:

- 문제를 구조적으로 정의하는 능력
- 잘못된 결과를 식별하는 능력
- 설계의 trade-off를 판단하는 능력

다만 한 가지는 분명합니다.

> 이 역할을 수행하는 사람의 수는 지금보다 줄어들 가능성이 높습니다.

### 결론

> "개발자는 어떤 역할로 재정의되는가?"

코드 작성은 점점 자동화되고 있습니다.
하지만 좋은 결과를 만드는 능력은 여전히 사람에게 있습니다.
