...
import { StockList, StockGrid, StockTile, decideWebviewSource } from '@/features/stock';
import { QueryErrorBoundary } from '@/shared/ui/QueryErrorBoundary';

// NOTE: Smart Parent / Dumb Children 패턴의 핵심.
// 이 컴포넌트가 스토어·React Query와 통신하고, 자식(List/Grid/Tile)은 props만 받는 순수 컴포넌트.
// onClick → handleStockClick: 웹뷰 시트 열기 (네이버 기본, 데이마켓은 설정 따라 야후/선택 모달)
...
export const StockViewSwitch = memo(() => {
  ...
  // 웹뷰 열기 — 데이마켓(US OVERNIGHT)에서만 설정에 따라 야후/모달 분기, 그 외 네이버.
  // price는 행(Row/Card/Tile)이 동봉 — prices 맵 의존 시 폴링마다 콜백 ref가 바뀌어 전 행 리렌더(PERF 주석).
  const dmSource = settings.daymarketWebviewSource;
  const handleStockClick = useCallback((sym: StockSymbol, price?: StockPrice) => {
    const decision = decideWebviewSource(sym, price, dmSource);
    if (decision === 'ask') setDaymarketAskSymbol(sym);
    else setDetailSymbol(sym, decision);
  }, [dmSource, setDetailSymbol, setDaymarketAskSymbol]);
