...
/** 섹터 데이터 — 활성 nation 기준 lazy fetch + 캐시. count = 표시 업종 수(설정값) */
export const useSectorData = (active: boolean, count = 20) => {
  ...
  // 표시 개수(count)가 바뀌면 캐시 무효화 — 다음 load에서 새 개수로 재요청
  useEffect(() => {
    setByNation({});
  ...
  const load = useCallback(async (target: SectorNation, force = false): Promise<boolean> => {
    ...
    setLoading(true);
    const data = await fetchSectors(target, count);
  ...
  const refreshAll = useCallback(async (): Promise<boolean> => {
    setLoading(true);
    const [dom, usa] = await Promise.all([fetchSectors('domestic', count), fetchSectors('USA', count)]);
    const now = Date.now();
