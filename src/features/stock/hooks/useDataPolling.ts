...
export const useDataPolling = (
  ...
  const domesticMarqueeRef = useRef<MarqueeItem[]>([]);
  const overseasMarqueeRef = useRef<MarqueeItem[]>([]);
  const mergeMarquee = useCallback(() => {
    setMarqueeItems([...domesticMarqueeRef.current, ...overseasMarqueeRef.current]);
  }, []);

  const queryClient = useQueryClient();

  // ── 국내 쿼리 ──
  // queryKey는 실제 fetch 대상과 일치: 국내 주식 + 국내 지수/선물
  const domesticCodes = [...classified.domesticStocks.map(s => s.code), ...classified.domesticIndexFutures.map(s => s.code)].sort();
  const domesticQuery = useQuery({
    queryKey: ['domestic', domesticCodes],
    queryFn: async () => {
      setProgress(0);
      const result = await fetchDomesticCycle(classifiedRef.current, setProgress);
      domesticMarqueeRef.current = result.marqueeItems;
      mergeMarquee();
      return result.prices;
    },
    enabled: true, // 마퀴는 종목 없어도 조회해야 함
    retry: false,
  ...
  const overseasQuery = useQuery({
    ...
    queryFn: async () => {
      const result = await fetchOverseasCycle(classifiedRef.current);
      overseasMarqueeRef.current = result.marqueeItems;
      mergeMarquee();
      return result.prices;
    },
    enabled: true,
